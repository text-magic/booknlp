...
class QuotationAttribution:
    ...
    def tag(self, quotes, entities, tokens):
        ...
        texts, metas, positions, global_entity_positions, quote_indexes = self.get_representation(
    ...
    def get_representation(self, quotes, entities, tokens, doLowerCase=True):
        ...
        for q_id, (start_tok, end_tok) in enumerate(quotes):
            ...
            while (
                start >= 0
                and count < window
                and wp_tok_count + len(self.model.tokenizer.tokenize(convert_word(tokens[start].text))) < 350
            ):
            ...
            while (
                ...
                and count < window
                and wp_tok_count + len(self.model.tokenizer.tokenize(convert_word(tokens[end].text))) < 475
            ):
                if in_quotes[end] == 0:
                    count += 1
            ...
            for i in range(start, end):
                ...
                # if the token ends a *different* quote, add an [ALTQUOTE] pseudo-token to represent it
                    ...
                    if q_end < end_tok:
                        ...
                        cands.append(
                            (
                                min(abs(q_end - start_tok), abs(q_start - end_tok)),
                                quotepos,
                                quotepos,
                                None,
                                "QUOTE",
                    ...
                    if in_quotes[entity_start] == 0:
                        for entity_end in entities_by_start[entity_start]:
                            ...
                            if entity_end < end and entity_end + inserts[entity_end - start] - start < len(toks):
                                ...
                                cands.append(
                                    (
                                        min(abs(entity_end - start_tok), abs(entity_start - end_tok)),
                                        entity_start + inserts[entity_start - start] - start,
                                        entity_end + inserts[entity_end - start] - start,
                                        None,
                                        "ENT",
                                        entity_start,
                                        entity_end,
                    ...
                    if " ".join(toks[s : e + 1]) != "[ALTQUOTE]":
                        ...
                        abs_positions.append(
                            (
                                ...
                                global_start,
                                global_end + 1,
                                " ".join([t.text for t in tokens[global_start : global_end + 1]]),
                            )
                        )
                    else:
                        abs_positions.append(
                            (
                                ...
                                altquote_map[s][0],
                                altquote_map[s][1],
                                " ".join([t.text for t in tokens[altquote_map[s][0] : altquote_map[s][1]]]),
                            )
                        )
