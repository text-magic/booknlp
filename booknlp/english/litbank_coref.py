...
class LitBankCoref:
    def __init__(self, model_file, gender_cats, pronominalCorefOnly=True):
        ...
        self.model = BERTCorefTagger(
            gender_cats=gender_cats, freeze_bert=True, base_model=base_model, pronominalCorefOnly=pronominalCorefOnly
        )
    ...
    def tag(self, tokens, g_ents, refs, ref_gender, attributed_quotations, quotes):
        ...
        assignments, global_entities = self.test(
            sentences, ents, max_words, max_ents, refs, ref_gender, attributed_quotations, quotes
        )
        return assignments
    ...
    def test(self, test_doc, test_ents, max_words, max_ents, refs, ref_gender, attributed_quotations, quotes):
        ...
        (
            ...
            test_transforms,
            test_quotes,
        ) = self.model.get_data(test_doc, test_ents, max_ents, max_words)

        assignments = self.model.forward(
            test_matrix,
            test_index,
            existing=refs,
    ...
    def convert_data(self, tokens, entities):
        ...
        for start, end, cat, text in entities:
            ...
            ent = Entity(
                w_in_sent_id_start, w_in_sent_id_end, in_quote=inQuote, quote_eid=None, entity_id=None, text=text
            )
