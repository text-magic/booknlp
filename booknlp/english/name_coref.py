...
class NameCoref:
                    ...
                    for l in range(k + 1, len(parts)):
                        ...
                        for m in range(l + 1, len(parts)):
                            ...
                            for n in range(m + 1, len(parts)):
                                variants[
                                    "%s %s %s %s %s %s" % (parts[i], parts[j], parts[k], parts[l], parts[m], parts[n])
                                ] = 1

                                for o in range(n + 1, len(parts)):
                                    variants[
                                        "%s %s %s %s %s %s %s"
                                        % (parts[i], parts[j], parts[k], parts[l], parts[m], parts[n], parts[o])
                                    ] = 1
    ...
    def process(self, spanFile):
        entities, is_named = self.read_file(spanFile)
        self.cluster(entities, is_named)
    ...
    def cluster_only_nouns(self, entities, refs, tokens):
        ...
        for start, end, cat, text in entities:
            ...
            for i in range(start, end + 1):
                ...
                if (hon_mapped is not None or (tokens[i].pos == "NOUN" or tokens[i].pos == "PROPN")) and tokens[
                    i
                ].text.lower()[0] != tokens[i].text[0]:
                    val = tokens[i].text
    ...
    def cluster(self, entities, is_named, refs):
        ...
        for ref in clusters:
            for ref2 in clusters:
                if (
                    ref == ref2
                    or clusters[ref] is None
                    or clusters[ref2] is None
                    or ref == -1
                    or ref2 == -1
                    or ref == 0
                    or ref2 == 0
