...
class Tagger(nn.Module):
    ...
    def forward_supersense(
        ...
        sequence_score1 = self.supersense_crf.score(
            torch.where(labels[0][:, 1:] == -100, torch.ones_like(labels[0][:, 1:]) * to_value, labels[0][:, 1:]),
            lens[0] - 2,
            logits=tag_space1,
        )
    ...
    def forward(self, input_ids, matrix1, matrix2, attention_mask=None, transforms=None, labels=None, lens=None):
        ...
        sequence_score1 = self.crf.score(
            torch.where(labels[0][:, 1:] == -100, torch.ones_like(labels[0][:, 1:]) * to_value, labels[0][:, 1:]),
            lens[0] - 2,
            logits=tag_space1,
        )
        ...
        sequence_score2 = self.crf.score(
            torch.where(labels[1][:, 1:] == -100, torch.ones_like(labels[1][:, 1:]) * to_value, labels[1][:, 1:]),
            lens[1] - 2,
            logits=tag_space2,
        )
        ...
        sequence_score3 = self.crf.score(
            torch.where(labels[2][:, 1:] == -100, torch.ones_like(labels[2][:, 1:]) * to_value, labels[2][:, 1:]),
            lens[2] - 2,
            logits=tag_space3,
        )
    ...
    def predict_all(
        self, wn, input_ids, attention_mask=None, transforms=None, lens=None, doEvent=True, doEntities=True, doSS=True
    ):
        def fix(sequence):
            """
        ...
        sequence_outputs, pooled_outputs, hidden_states = self.bert(
            input_ids, token_type_ids=None, attention_mask=attention_mask, output_hidden_states=True, return_dict=False
        )
        if self.num_layers == 4:
            all_layers = torch.cat((hidden_states[-1], hidden_states[-2], hidden_states[-3], hidden_states[-4]), 2)
        elif self.num_layers == 2:
        ...
        if doSS:
            ...
            all_supersense_tags1, all_index1, all_missing1, n_lens1 = get_supersense_layer_transformation(
                tag_space1, t1
            )
    ...
    def tag_all(
        ...
        with torch.no_grad():
            for b in range(len(batched_data)):
                all_tags1, all_tags2, all_tags3, event_logits, all_supersense_tags1 = self.predict_all(
                    batched_wn[b],
                    batched_data[b],
                ...
                if doEntities:
                    for d in range(len(all_tags1)):
                        ...
                        for entity in self.get_spans(
                            self.rev_tagset, c, all_tags1[d], batched_orig_token_lens[b][d], batched_sents[b][d][1:]
                        ):
                            preds[entity] = 1
                        for entity in self.get_spans(
                            self.rev_tagset, c, all_tags2[d], batched_orig_token_lens[b][d], batched_sents[b][d][1:]
                        ):
                            preds[entity] = 1
                        for entity in self.get_spans(
                            self.rev_tagset, c, all_tags3[d], batched_orig_token_lens[b][d], batched_sents[b][d][1:]
                        ):
                            preds[entity] = 1
    ...
    def tag(self, batched_sents, batched_data, batched_mask, batched_transforms, batched_orig_token_lens, ordering):
        ...
        with torch.no_grad():
            for b in range(len(batched_data)):
                all_tags1, all_tags2, all_tags3 = self.predict(
                    batched_data[b],
                    attention_mask=batched_mask[b],
                    transforms=batched_transforms[b],
                    lens=batched_orig_token_lens[b],
    ...
    def supersense_evaluate(
        ...
        test_batched_mask,
        test_batched_labels,
        test_batched_transforms,
        test_batched_layered_labels1,
        test_batched_layered_labels2,
        test_batched_layered_labels3,
        test_batched_layered_labels4,
        dev_lens,
    ):
        self.eval()
        ...
        with torch.no_grad():
            ...
            for b in range(len(test_batched_data)):
                all_tags1 = self.supersense_predict(
                    test_batched_wn[b],
                    test_batched_data[b],
                    attention_mask=test_batched_mask[b],
                    transforms=test_batched_transforms[b],
                    lens=dev_lens[0][b],
                )
                ...
                for d in range(len(all_tags1)):
                    ...
                    for entity in self.get_spans(
                        self.rev_supersense_tagset,
                        c,
                        test_batched_layered_labels1[b][d][1:],
                        dev_lens[0][b][d],
                        test_batched_sents[b][d][1:],
                    ):
                        golds[entity] = 1

                    # predicted tags already have [CLS] removed
                    for entity in self.get_spans(
                        self.rev_supersense_tagset, c, all_tags1[d], dev_lens[0][b][d], test_batched_sents[b][d][1:]
                    ):
                        preds[entity] = 1
            ...
            for b in range(len(test_batched_data)):
                ...
                for d in range(len(all_tags1)):
                    ...
                    for entity in self.get_spans(
                        self.rev_tagset,
                        c,
                        test_batched_layered_labels2[b][d][1:],
                        dev_lens[0][b][d],
                        test_batched_sents[b][d][1:],
                    ):
                        golds[entity] = 1
                    for entity in self.get_spans(
                        self.rev_tagset,
                        c,
                        test_batched_layered_labels3[b][d][1:],
                        dev_lens[0][b][d],
                        test_batched_sents[b][d][1:],
                    ):
                        golds[entity] = 1
                    for entity in self.get_spans(
                        self.rev_tagset,
                        c,
                        test_batched_layered_labels4[b][d][1:],
                        dev_lens[0][b][d],
                        test_batched_sents[b][d][1:],
                    ):
                        golds[entity] = 1
                    ...
                    for entity in self.get_spans(
                        self.rev_tagset, c, all_tags1[d], dev_lens[0][b][d], test_batched_sents[b][d][1:]
                    ):
                        preds[entity] = 1
                    for entity in self.get_spans(
                        self.rev_tagset, c, all_tags2[d], dev_lens[0][b][d], test_batched_sents[b][d][1:]
                    ):
                        preds[entity] = 1
                    for entity in self.get_spans(
                        self.rev_tagset, c, all_tags3[d], dev_lens[0][b][d], test_batched_sents[b][d][1:]
                    ):
                        preds[entity] = 1
    ...
    def evaluateFlat(
        self, dev_batched_data, dev_batched_mask, dev_batched_labels, dev_batched_transforms, metric, tagset
    ):
        ...
        with torch.no_grad():
            ...
            for b in range(len(dev_batched_data)):
                logits = self.forwardFlatSequence(
                    dev_batched_data[b],
                    token_type_ids=None,
                    attention_mask=dev_batched_mask[b],
                    transforms=dev_batched_transforms[b],
                )
