...
class BERTSpeakerID(nn.Module):
    ...
    def get_batches(self, all_x, all_m, batch_size=32, doLowerCase=True):
        ...
        for i in range(0, len(all_x), batch_size):
            ...
            for j, (eid, cands, quote) in enumerate(mb):
                ...
                for l in range(10):
                    for k in range(e1_start_wp, e1_end_wp):
                        matrix_quote[l][k] = 1.0 / (e1_end_wp - e1_start_wp)
                ...
                for c_idx, (start, end, truth, cand_eid) in enumerate(cands):
                    ...
                    for k in range(e2_start_wp, e2_end_wp):
                        matrix_cands[c_idx][k] = 1.0 / (e2_end_wp - e2_start_wp)
            ...
            batches_x.append(
                {
                    "toks": torch.LongTensor(current_batch_input_ids).to(device),
                    "mask": torch.LongTensor(current_batch_attention_mask).to(device),
                }
            )
            batches_m.append(
                {
                    "cands": torch.FloatTensor(np.array(current_batch_matrix_cands)).to(device),
                    "quote": torch.FloatTensor(np.array(current_batch_matrix_quote)).to(device),
                }
            )
            batches_y.append(
                {
                    "y": torch.LongTensor(current_batch_y).to(device),
                    "eid": current_batch_eid,
                    "quote_eids": current_quote_eids,
                }
            )
                    ...
                    if pred[0] < len(meta[idx][1]):
                        ...
                        if epoch == "test":
                            print(
                                ...
                                " ".join(sent[:ent_start]),
                                PINK,
                                " ".join(sent[ent_start:ent_end]),
                                "(%s)" % int(val.detach().cpu().numpy()),
                                ENDC,
                                " ".join(sent[ent_end:]),
                            )
