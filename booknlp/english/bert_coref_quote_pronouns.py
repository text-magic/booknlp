...
class BERTCorefTagger(nn.Module):
    ...
    def get_mention_reps(
        ...
        _, pooled_outputs, sequence_outputs = self.bert(
    ...
    def forward(
        ...
        for b in range(len(matrix)):
            ...
            span_reps = self.get_mention_reps(
        ...
        unary_scores = self.unary3(
            self.tanh(
                self.drop_layer_020(self.unary2(self.tanh(self.drop_layer_020(self.unary1(span_representation)))))
            )
        )
        ...
        for inQuoteVal in [False, True]:
            for i in range(num_mentions):
                ...
                if len(cands_idx) == 0:
                    ...
                    if (
                        entity.quote_mention is not None
                        and assignments[entity.quote_mention] is not None
                        and (
                            entity.text.lower() == "i"
                            or entity.text.lower() == "me"
                            or entity.text.lower() == "my"
                            or entity.text.lower() == "myself"
                        )
                        and entity.in_quote
                    ):
                    ...
                    for cand in cands_idx:
                        if (
                            entity.global_start >= entities[cand].global_start
                            and entity.global_end < entities[cand].global_end
                        ):
                            nest1.append(1)
                        else:
                            nest1.append(0)
                        if (
                            entities[cand].global_start >= entity.global_start
                            and entities[cand].global_end < entity.global_end
                        ):
                            nest2.append(1)
                        else:
                            nest2.append(0)
                    ...
                    preds = self.mention_mention3(
                        self.tanh(
                            self.drop_layer_020(
                                self.mention_mention2(self.tanh(self.drop_layer_020(self.mention_mention1(concat))))
                            )
                        )
                    )
                                ...
                                if preds[cand_idx] > 0:
                                    ...
                                    if cand_assignment is None:
                                        print(
                                            ...
                                            inQuoteVal,
                                            assignments,
                                            torch.sort(preds, descending=True),
                                        )
                                        sys.exit(1)

                                    if self.is_compatible(
                                        cand_mention, cand_assignment, entity_properties, entity, ref_genders, score
                                    ):
    ...
    def read_toks(self, filename):
        ...
        with open(filename) as file:
            ...
            for line in file:
                ...
                tok = Token(
                    parID, sentenceID, tok_sent_idx, tokenID, text, pos, None, lemma, deprel, dephead, ner, startByte
                )
        ...
        with open(filename, encoding="utf-8") as file:
            for line in file:
                if line.startswith("#begin document"):
                    ...
                    doc_antecedent_labels, big_ents, max_words, max_ents, quote_antecedent_labels = self.get_ant_labels(
                        all_doc_sents, all_doc_ents, all_doc_quotes
                    )
                    ...
                    for c in coref:
                        if c.startswith("(") and c.endswith(")"):
                            ...
                            ents[(tid, tid)] = Entity(
                                ...
                                in_quote=inQuote[0],
                                quote_eid=inQuote[1],
                                quote_id=len(adjusted_quotes),
                                entity_id=c,
                                text=" ".join(sent[tid : tid + 1]),
                            )
                            ...
                            ents[(start_tid, tid)] = Entity(
                                ...
                                quote_id=len(adjusted_quotes),
                                entity_id=c,
                                text=" ".join(sent[start_tid : tid + 1]),
                            )
